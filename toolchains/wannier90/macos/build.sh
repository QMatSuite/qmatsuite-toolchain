...
# IMPORTANT: Uses Apple Accelerate framework for BLAS/LAPACK (NOT OpenBLAS)
...
if grep -q "openblas" ./make.inc; then
    echo "WARNING: make.inc contains OpenBLAS references. macOS should use Accelerate framework!"
fi
if ! grep -q "Accelerate" ./make.inc; then
    echo "WARNING: make.inc does not reference Accelerate framework!"
fi
