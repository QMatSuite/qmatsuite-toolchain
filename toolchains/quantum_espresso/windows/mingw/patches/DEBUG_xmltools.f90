...
CONTAINS
  ...
  SUBROUTINE get_i_attr ( attrname, attrval_i )
    ...
    if ( len_trim(attrval_c) > 0 ) then
       ! Debug: print raw attribute value before conversion
       print '("DEBUG get_i_attr: attrname=[",A,"], raw_value=[",A,"]")', &
            trim(attrname), trim(attrval_c)
       READ (attrval_c,*, iostat=iostat_val) attrval_i
       if (iostat_val == 0) then
          print '("DEBUG get_i_attr: converted to integer=",I0)', attrval_i
          return
       else
          print '("ERROR reading attribute ",a,": expected integer, found [",a,"], iostat=",I0)', &
               trim(attrname), trim(attrval_c), iostat_val
       end if
    else
       print '("WARNING get_i_attr: attrname=[",A,"] not found or empty, defaulting to 0")', &
            trim(attrname)
    end if
    attrval_i = 0
    !
...
1      print '("Error reading attribute ",a,": expected real, found ",a)', &
  ...
  SUBROUTINE get_c_attr ( attrname, attrval_c )
    ...
    if ( .not. allocated(attrlist) ) then
       print '("DEBUG get_c_attr: attrname=[",A,"], attrlist not allocated")', trim(attrname)
       return
    end if
    if ( len_trim(attrlist) < 1 ) then
       print '("DEBUG get_c_attr: attrname=[",A,"], attrlist is empty")', trim(attrname)
       return
    end if
    ! Debug: print attrlist contents
    print '("DEBUG get_c_attr: attrname=[",A,"], attrlist=[",A,"]")', &
         trim(attrname), trim(attrlist(1:MIN(200, LEN(attrlist))))
    !
    j0 = 1
